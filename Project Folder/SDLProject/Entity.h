...
enum EntityType { PLATFORM, PLAYER, ENEMY, BACKGROUND, LIVES, ROCK, LADDER};
enum AIType     { WALKER, GUARD, FlYER };
enum AIState    { WALKING, IDLE, ATTACKING };


enum AnimationDirection { LEFT, RIGHT, UP, DOWN, ATTACK};
...
private:
    ...
    int m_walking[5][6]; // 4x4 array for walking animations
    ...
    // Add this member to store tile index for different rocks
    int m_tile_index;
...
public:
    ...
    Entity(GLuint texture_id, int tile_index, float width, float height, EntityType EntityType); // Added for rocks
    ...
    void draw_static_sprite(ShaderProgram* program, GLuint texture_id, int tile_index);
    
    float get_uv_offset(int tile_index);
    
    bool const check_collision(Entity* other) const;
    
    void delte_life(int current_lives);
    ...
    void face_down() { m_animation_indices = m_walking[UP]; }
    void face_attack() {m_animation_indices = m_walking[ATTACK]; } // Added for attack animation
    ...
    void player_attack() {face_attack();} // Added for attack
    ...
    // Enemy movements
    void ai_follow_player(Entity *player);
    void ai_vertical_walker(float upperLimit, float lowerLimit);
    void ai_horizontally_walker(float leftLimit, float rightLimit);
    void ai_fly_across_screen();
    ...
    void set_walking(int walking[5][6])
    {
        for (int i = 0; i < 5; ++i)
        {
            for (int j = 0; j < 6; ++j)
