...
void ShaderProgram::set_is_level_d(bool value)
{
    glUseProgram(m_program_id);
    GLint location = glGetUniformLocation(m_program_id, "onlyLevel_D");
    if (location == -1) {
        std::cerr << "Warning: Uniform 'onlyLevel_D' not found in shader program." << std::endl;
    } else {
